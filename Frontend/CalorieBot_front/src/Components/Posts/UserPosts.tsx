...
function UserPosts() {
    ...
    const getPosts = async (url: string) => {
        ...
        try {
            ...
            const response = await authenticatedFetch(`${url}${separator}user=${user.id}`, {
                method: "GET",
            });
    ...
    return (
        <>
            <div className="posts-wrap" style={{ color: theme.text_plain }}>
                {posts.map((post: any) => (
                    <Post key={post.id} post={post} formatTimestamp={formatTimestamp} onLikeToggle={handleLikeToggle} />
                ))}
